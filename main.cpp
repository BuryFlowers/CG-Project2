		...
		for (int j = 0; j < lightPath.size(); j++) {
			...
			if (visible) {
				...
				for (int k = 1; k < j; k++) {
					...
					wi = normalize(wi);
					vec3 wo = k == j ? normalize(cameraPath[0].ip.p - lightPath[j].ip.p) : normalize(lightPath[k + 1].ip.p - lightPath[k].ip.p);
					vec3 BRDF = lightPath[k].ip.mat->phongModelBRDF(wi, wo, lightPath[k].ip.n, lightPath[k].ip.uv);
					float G = max(dot(lightPath[k].ip.n, wi), 0.0f) / len;
					currentResult = currentResult * G * BRDF;
				...
				for (int k = i; k >= 0; k--) {
					...
					float G = max(dot(cameraPath[k].ip.n, wi), 0.0f) / len;
					currentResult = currentResult * G * BRDF;
